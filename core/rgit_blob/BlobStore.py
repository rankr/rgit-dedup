...
class BlobStore:
	'''
	256 sub-directories in each blob_store_path
	256 index files in each sub-directories
	one to_write file in each sub-directories which indicate where to write next
	many main files to store blobs
	
	structure of to_write: which,offset,already (which main file it should write next,the size already wrote in the main file,how many blobs it had write in the main file)
	...
	'''
	def __init__(self, blob_store_path):
		self.base_path = os.path.abspath(blob_store_path)
		self.blob_init()
	...
	def absorb(self, triples):
		...
		for i in xrange(0, len(triples)):
			...
			if base == '':#it is not delta
				triples[i][2] = int2msb_blob(len(data), False) + data
			else:#it is delta
				triples[i][2] = int2msb_blob(len(data), True) + base + data
		...
		for i in suffix:
			...
			if not write_list[i]:
				continue
			main_file = open(os.path.join(self.base_path, i, 'rgit_blob_main%d'%(write_list[i][0][2])), 'ab')
			for posi, sha, which, offset in write_list[i]:
				if last_which != which:
					last_which = which
					main_file.close()
					main_file = open(os.path.join(self.base_path, i, 'rgit_blob_main%d'%(which)), 'ab')
	...
	def blob_init(self):
		...
		for i in suffix:
			...
			if 'to_write' not in files:
				...
				w.close()
			if 'rgit_blob_main0' not in files:
				w = open(os.path.join(dir_path, 'rgit_blob_main0'), 'wb')
				w.close()
			for j in suffix:
	...
	def cat_blob(self, sha):
		...
		if which == -1:
			return -1
		f = open(os.path.join(self.base_path, sha[0:2], 'rgit_blob_main%d'%(which)), 'rb')
		f.seek(offset)
		compressed, base = read_main_blob(f)
		f.close()
		if not base:#not delta
			return zlib.decompress(compressed)
		else:
			father_data = self.cat_blob(base)
